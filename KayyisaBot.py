...
def start(bot, update):
  update.message.reply_text(
    "Assalamu'alaikum {}!"
    "\nSelamat datang di PermadiBot."
    "\nPermadiBot akan membantumu untuk menampilkan jadwal shalat, ayat Al-Quran dan do'a sehari-hari."
    ...
    "\nKirim perintah /help untuk bantuan".format(
...
def shalat(bot, update, args):
  try:
    ...
    update.message.reply_text(
      ...
      .format(date, fajr, sunrise, dhuhr, asr, sunset, maghrib, isha, imsak,
              midnight))
...
def quran(bot, update, args):
  try:
    ...
    if match is not None:
      ...
      r = requests.get(
        'http://api.fathimah.ga/quran/format/json/surat/{}/ayat/{}'.format(
          s, a))
      data = r.content
...
def cari(bot, update, args):
  try:
    kata = args[0].lower()
    r = requests.get(
      'http://api.fathimah.ga/quran/format/json/cari/{}/bahasa/id/mulai/0/limit/100'
      .format(kata))
    ...
    if qs == '':
      update.message.reply_text(
        'Tidak ada hasil pencarian untuk kata {}'.format(kata))
    else:
...
def catatan(bot, update, args):
  try:
    ...
    if no in range(1, 1611):
      r = requests.get(
        'http://api.fathimah.ga/quran/format/json/catatan/{}'.format(no))
      data = json.loads(r.content)
      ...
      update.message.reply_text(
        'Tidak terdapat catatan depag dengan nomor {}.'.format(no))

  except (IndexError, ValueError):
    update.message.reply_text(
      'Kirim perintah /catatan <nomor> untuk mengetahui catatan depag mengenai terjemahan ayat tertentu.'
      '\nContoh :\n/catatan 5\n/catatan 256')
  ...
  try:
    ...
    if kode in range(0, 20):
      ...
      update.message.reply_text(
        ...
        .format(name, ref, pronun, trans, hadith))
    else:
      update.message.reply_text(
        "Nomor {} tidak terdapat dalam daftar do'a. Hanya terdapat 20 do'a. Untuk lihat daftarnya, kirim perintah /doa"
        .format(arg))
  except (IndexError, ValueError):
    update.message.reply_text(
...
def alarm(bot, job):
  ...
  bot.send_message(
    job.context,
    text="Assalamu'alaikum! Ayat pilihan hari ini adalah QS.{}:{}".format(
      s, a))
  ...
  if match is not None:
    ...
    akhir = int(akhir) - int(awal) + 1
    r = requests.get(
      'http://api.fathimah.ga/quran/format/json/surat/{}/ayat/{}'.format(s, a))
    data = r.content
    data = json.loads(data)
    for i in range(akhir):
      ayat = data['ayat']['data']['ar'][i]['teks'].encode('utf-8')
...
def main():
  updater = Updater('5748269951:AAHehu4ZpC4ailVb3yf_F1Mkhgor5ssit-g')
